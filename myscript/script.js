...
$("#btnaddtoFav").on("click", function() {
  var currentSvg = $(this).data("currentSvg");
  var newSvg = '';

  if (currentSvg === "original") {
    newSvg = `
    ...
    `;
    $(this).data("currentSvg", "new");
  } else {
    // Change this to the original SVG markup or empty string if needed
    newSvg = `
    <svg class="w-6 h-6 text-gray-800 dark:text-black  md:mt-0" aria-hidden="true" xmlns="http:www.w3.org/2000/svg" fill="none" viewBox="0 0 21 19">
    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="0.8" d="M11 4C5.5-1.5-1.5 5.5 4 11l7 7 7-7c5.458-5.458-1.542-12.458-7-7Z"/>
    </svg>
    `;
    $(this).data("currentSvg", "original");
  }
...
//fetch data from api
fetch('/jsondata/cardData.json').then(res =>
res.json()).then(data=>{
    const productList = data.flashSale
